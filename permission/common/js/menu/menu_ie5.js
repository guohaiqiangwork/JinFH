...
var nWidth=document.body.clientWidth-150; 
...
{ 	
  ...
  img.src="/reins/images/jiantou copy-2.gif"; 
...
{ 	
  ...
  img.src="/reins/images/jiantou copy.gif"; 
...
function goToUrl(cUrl,cTarg){
	...
	if(cUrl.indexOf("mailto:")!=-1){ document.location=cUrl; }
		...
		if(cTarg=="_self"){self.document.location=cUrl;}
			...
			if(typeof top[cTarg]!="undefined"){top[cTarg].document.replace=cUrl}
...
mn+='<div id="master" style="position:absolute\;left:150px\;top:0px\;z-index:'+zORDER+'">';
...
if(remoteTRI0802RING) { mnDisplay='display:none'; }
...
function keyListen(DnEvents) {
	...
	if(eka||eks||ekc||remoteTRI0802RING){return false;}
...
function activateMenu(mmNum) {
	...
	if(typeof sP[mmNum]=="undefined"||!remoteTRI0802RING) { return false; }
