/**********************  COTT - V1.2   ***************************
*	@author 		COTT
*	@email 			WENJIADONG@Sinosoft.com.cn(全小写)
*	@description 	本日历的年份限制是（1000 - 9999）
*					按ESC键关闭该控件
*					在年和月的显示地方点击时会分别出年与月的下拉框
*					在年和月的显示地方点击时会分别出年与月的下拉框
*					控件外任意点击一点即可关闭该控件
*   @how user   : 	一、<input type=text id=text2 ><input type=button value="Show Calendar" click="showCalendar(this,document.all.text2)">
				  	二、<input type=text id=text1 click="showCalendar(this)">
				  	(想看提示：请再568行测试【将鼠标悬停再变量上面】，测试前需再click前添加on 为防止出现惊叹号，请测试完效果，将on取除了)
'**************************************************************************/
...
function wpCalendar(){
	...
	this.datestyle	= "yyyy-MM-dd";//added in Ver 1.1
	this.cellwidth	= 21;//设置单个单元格的宽度
	this.cellheight	= 20;//设置单元格的高度
	this.versioninfo= "COTT-Canlendar Version:1.2;Author:COTT";//版本信息
	this.tgtObject	= null;
	...
	this.MonthinMM	= this.datestyle.indexOf("MM")>=0?true:false;//new added Version:1.1
	...
	try{this.IE6	= (parseFloat(window.navigator.appVersion.match(/MSIE (\d+\.\d+)/)[1])>=5.5);}catch(e){this.IE6 = false;}
	...
	this.writeCalendar=function (){	//cott-2014-09-10主要复写程序**********
		var yy = this.year;
		var mm = this.month;
		//写入头部的年月
...
function showCalendar() //主调函数
{
	...
	var twid  = tt.clientWidth;//TT控件本身的宽	//added in Ver 1.1
	...
	//根据当前输入框的日期显示日历的年月
	var reg = new RegExp("^" + wpCalendar.datestyle.replace((wpCalendar.Dateindd?"dd":"d"),"(\\d{1,2})").replace("yyyy","(\\d{4})").replace((wpCalendar.MonthinMM?"MM":"M"), "(\\d{1,2})")+"$");	//added in Ver 1.1
	var r = wpCalendar.tgtObject.value.match(reg); 
	if(r!=null){
		var t=getIndex();
		r[t[2]]=r[t[2]]-1; 
		if(wpCalendar.datestyle.indexOf("d")<0)r[t[3]]=1;			//使用一个"d"就可以包括两种情况	 //added in Ver 1.1
	...
	function getIndex(){//根据日期格式设置返回年月日分别所在的位置 //added in Ver 1.1
		var i,j;
