...

{
    ...
    //modify by zhanghaisheng begin 20080303 修改日期校验错误的问题
    var   reg   =   /^(\d{4})-(\d{1,2})-\d{1,2}$/; 
    if(arr=strValue.match(reg)){   
    if(arr   ==   testDate.getFullYear()   &&   arr   ==   (testDate.getMonth()+1))return   false;   
    testDate.setDate(0); 
     if(arr   ==   testDate.getFullYear()   &&   arr   ==   (testDate.getMonth()+1))return   false;   
  }   
    return   true;
    
    //modify by zhanghaisheng end
