 ...
 * @author user
 */
			YAHOO.namespace("Arch4platform");

			YAHOO.Arch4platform.DataSource = function(aData, oConfigs){
...
//						YAHOO.Arch4platform.DataSource.superclass.constructor.call(this,_dataset,oConfigs);
				        this.data = _dataset;
				        this._init();
			...
			YAHOO.Arch4platform.DataSource.prototype = new YAHOO.widget.DS_JSArray();
//			YAHOO.lang.extend(YAHOO.Arch4platform.DataSource,YAHOO.widget.DS_JSArray);

			YAHOO.Arch4platform.selectUI = function(input, container, hidden, type, dataset, oConfig){
				if(arguments.length>0){
					this._oConfigs = oConfig;
					//constructs autoComplete.
					YAHOO.Arch4platform.selectUI.superclass.constructor.call(this,input,container,dataset,oConfig);
			...
			YAHOO.lang.extend(YAHOO.Arch4platform.selectUI,YAHOO.widget.AutoComplete);

YAHOO.Arch4platform.selectUI.prototype._stopEvent = false;


YAHOO.Arch4platform.selectUI.prototype._onTextboxFocus = function (v,oSelf) {
...
YAHOO.Arch4platform.selectUI.prototype._onHiddenChange = function(v,oSelf){
...
YAHOO.Arch4platform.selectUI.prototype.checkInputValue = function(oAC){
...
YAHOO.Arch4platform.selectUI.prototype.changeInputValue = function(oAC){
	//alert("aac");
	if(oAC.dwrfname!=undefined && oAC.dwrfname!='') return;
	var elInput  = oAC._oTextbox;
	var elHidden = oAC.hidden;
...
function trim(str) {
	return str.replace(/^\s*(.*?)[\s\n]*$/g, '$1');
			...
			YAHOO.Arch4platform.selectUI.prototype.initInputValue = function(oAC){
				if(oAC.dwrfname!=undefined && oAC.dwrfname!='') return;
				var elInput  = oAC._oTextbox;
				var elHidden = oAC.hidden;
			 ...
			 * @param {Object} bShow
			 */
			YAHOO.Arch4platform.selectUI.prototype._toggleTips = function(sTips,bShow){
...
//						this.oTip.style.width = ow+"px";
			...
			YAHOO.Arch4platform.selectUI.prototype._onTextboxKeyUp = function(v,oSelf) {
			...
			YAHOO.Arch4platform.selectUI.prototype.numPar=/\d{1,}$/;
			YAHOO.Arch4platform.selectUI.prototype.sendQuery = function(sQuery){
				...
				if(YAHOO.lang.isUndefined(this.dwrfname)){
				    ...
				    var responseSuccess = function(oResp) {
						...
						results = new YAHOO.Arch4platform.DataSource(oResp,oAC._oConfigs);
...
//				       	}
			 ...
			 * @param {Object} oSelf
			 */
			YAHOO.Arch4platform.selectUI.prototype.cleanHidden = function(v,oSelf){
			...
			YAHOO.Arch4platform.selectUI.prototype.formatResult = function(oResultItem, sQuery) {
	...
	for(var j=0;j<childs.length;j++){
		...
		if(node.tagName=="DIV"){
			...
			if(node.className=="selectConfig"){
				var cfgs = YAHOO.util.Dom.getElementsBy(function(e){code[e.className]=e.innerHTML.trim();return true},"div",node);
			}
...
function initMutiSelect(index,selectConfig){
	...
	if(selectConfig.type=="firstLoad"){
		...
		var ds = new YAHOO.Arch4platform.DataSource([],oConfig);
		var ac = new YAHOO.Arch4platform.selectUI(selectConfig.field,selectConfig.field+"_Container",selectConfig.hidden,'list',ds,oConfig);
...
function initSelect(codes){
	...
	var ds = new YAHOO.Arch4platform.DataSource(codes.dataSet,oConfig);

	var ac = new YAHOO.Arch4platform.selectUI(codes.field,codes.field+"_Container",codes.hidden,'list',ds,oConfig);
...
function replaceSelect(id,zIndex){
	...
	//yui初始化得时候没有给其中一个input赋上name的值，导致加减条操作时有问题，暂时加入以下一行，以后需要平台组修改
	selectInput.name=el.name+"_ac";
	if(el.disabled != undefined){
		selectInput.disabled=el.disabled;
	}
...
function initRemoteCode(codes, language){
	...
	var data;
	var ds = new YAHOO.Arch4platform.DataSource(data,oConfig);
	var ac = new YAHOO.Arch4platform.selectUI(codes.field,codes.field+"_Container",codes.hidden,'list',ds,oConfig);
    _defaultRemoteSelectConfig(ac);
}
...
var callbackGetAllCode = function(inputObject,outputObject,returnObject){
	for(var i=0;i<inputObject.length;i++){
		...
		if(ro.type=="firstLoad"){
			...
			var ds = new YAHOO.Arch4platform.DataSource(data,oConfig);
			var ac = new YAHOO.Arch4platform.selectUI(ro.field,ro.field+"_Container",ro.hidden,'list',ds,oConfig);
			 _defaultLocalSelectConfig(ac);
