...
function insertInnerRowForStruts1(pageCode,innerPageCode,docid,innerIndex,field){
	var parentindex = field.name.substring(field.name.indexOf("[")+1,field.name.indexOf("]"))
	var doc = document.getElementById(docid);
	var gi = Number(doc.value);
	var index = gi;
	...
	var rowsCount = tbody.rows.length;
  	for(var i=0;i<rowsCount;i++){
		...
		document.body.removeChild(tdiv);
		//alert( pageCode+"["+gi+"]."+innerPageCode );
		document.all(pageCode+"["+parentindex+"]."+innerPageCode).tBodies.item(0).appendChild(row);
    }
    innergi++;
	globalindex[pageCode+"_"+index+"_"+innerPageCode] = innergi;
	doc.value= gi+1;
...
function initAddSelect(pageCode,index){
	var divName=pageCode+"["+index+"]";
	addSelect(divName);
}
