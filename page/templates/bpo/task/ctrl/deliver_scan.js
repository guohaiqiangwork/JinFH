    ...
    ($,table_data,http,dialog,bpo_service, util_service, route_config,bootstrapTable,editable,bootbox) => {
        ...
        */
        ...
        initAjaxEdit() {
            ...
            let customerCode = localStorage.getItem('customerCode');
            bpo_service.getFileByStatus(customerCode,'02',$.proxy(this.initSearch, this));
        },
        initSearch(data){
             var dataList={
                messages:"",
             ...
             $('#search-input').bsSuggest({
                ...
                http.post({
                    apiName:'file_new',
                    data:{barCode:data.barCode,customerCode:localStorage.getItem('customerCode'),jiaojieTaskId:route_config.getStateParams()['taskId'],status:'03',jiaojieMan:localStorage.getItem('userCode')},
                    success(result) {
                         //alert(data);
                         $('#search-input').val("");
                         let input = $('#selectArea');
                         input.val(input.val()+data.barCode+"移交成功!"+"\n");
