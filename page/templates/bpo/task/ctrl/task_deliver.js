    ...
    ($,table_data,http,dialog,bpo_service, util_service, route_config,bootstrapTable,editable,bootbox) => {
        ...
        deleteClick(event){
            ...
            dialog.confirm({
                ...
                onSuerBefore() {
                    http.post({
                       apiName:'file_new',
                       data:{barCode:barCode,customerCode:localStorage.getItem('customerCode'),jiaojieTaskId:"",status:'02'},
                        success(data) {
                            dialog.alert({
                                content: '移除成功。'
                            });
        ...
        */
        render(data, flag){
            if (!data && !flag)
                    return;
            var deliverTime = new Date(data.deliverTime).Format("yyyy-MM-dd");
            var realDeliverTime= new Date(data.realDeliverTime).Format("yyyy-MM-dd");
            data.deliverTime = deliverTime;
            data.realDeliverTime = realDeliverTime;
        ...
        initAjaxEdit() {
            ...
            $("#creater").editable();
            $("#deliverTime").editable();
            $("#realDeliverTime").editable();
            $("#owner").editable();
            $("#contactWay").editable();
        },
        columns1: [{
